export const CONFIG = {
  gravity: 100, defaultPull: 1, horizon: 1.35, softening: 0.55,
  maxAcceleration: 85, step: 1 / 120, drag: 0.006,
  launchStrength: 1.5, maxPull: 8, predictionSteps: 840,
  trailLength: 100, trailInterval: 1 / 30, captureDuration: 0.65,
  escapeRadius: 65, maxObjects: 32, spawnRadius: 9,
  visuals: {
    diskSpeed: 0.8,
    diskTurbulence: 0.72,
    haloStrength: 0.14,
    bloomStrength: 0.36,
    bloomThreshold: 1.12,
    shadowScale: 1.06,
    criticalScale: 1.04,
    diskInnerRadius: 1.48,
    diskOuterRadius: 4.8,
    diskInnerFadeWidth: 0.16,
    diskLensOuterRadius: 3.6,
    diskLensSpread: 0.38,
    diskLensOpacity: 0.92,
    objectLensInner: 1.05,
    objectLensOuter: 2.8,
    objectLensOpacity: 0.62,
    objectLensMinScale: 0.35,
    objectLensMaxScale: 0.95,
    backgroundLensStrength: 0.06,
    lensingExtent: 3.2,
  },
};
