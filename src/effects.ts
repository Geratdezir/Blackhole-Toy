...
export function updateToy(toy: Toy, dt: number) {
  ...
  // Physical stretching begins inside the optical zones, not near the outer disk.
  const stretch = 1 + Math.max(0, 1 - (r - C.horizon) / 1.2) * 2.2;
  ...
  mesh.traverse(o => { if (o instanceof T.Mesh) (o.material as T.MeshStandardMaterial).opacity = shrink; });
  toy.trailClock += dt;
